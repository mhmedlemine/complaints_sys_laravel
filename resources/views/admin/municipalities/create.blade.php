...
@section('content')
    <h2 class="text-2xl font-semibold mb-4">Create Municipality</h2>
    <form action="{{ route('admin.municipalities.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="moughataa_id" name="moughataa_id" required>
                @foreach($moughataas as $moughataa)
                    <option value="{{ $moughataa->id }}">{{ $moughataa->name }}</option>
                @endforeach
            </select>
        </div>
